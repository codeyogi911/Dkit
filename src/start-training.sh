...
source activate tensorflow_p36 && \
cd Dkit/src && python3 dkit.py train --dataset=../datasets/IKEA_FIXA --weights=imagenet && \
cd ../logs && cd $(ls -td -- */ | head -n 1) && aws s3 sync mask_rcnn_dkit_0030.h5 s3://dkitmodels-sg/mask_rcnn_dkit_latest.h5 
EOF
